...
void ImGui::EndGroupBox()
{
	...
	if (window->ScrollbarY) {
		...
		if (window->Scroll.y > 1.0f) {
			...
			RenderText(ImVec2(maxTop.x - 18, maxTop.y - 16), "");
		}
		...
		if ((GetScrollMaxY() - window->Scroll.y) > 1.f) {
			...
			RenderText(ImVec2(maxBottom.x - 18, maxBottom.y - 18), "");
		}
	}
...
bool ImGui::BeginChildcustom(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags, int i)
{
    ImGuiWindow* window = GetCurrentWindow();
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2 & size_arg, bool border, ImGuiWindowFlags flags, int i)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGuiContext& g = *GImGui;
    ImGuiWindow* parent_window = g.CurrentWindow;
...
bool ImGui::BeginEndBorder(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			//auto innerBorderColor = ImColor(40, 40, 40);
...
bool ImGui::BeginGroupBoxMain(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		{
			auto backgroundColor = ImColor(0, 0, 0);
			auto innerBorderColor = ImColor(0, 0, 0);
			auto outerBorderColor = ImColor(0, 0, 0);
