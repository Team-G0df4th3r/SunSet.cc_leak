...
bool ImGui::Tab(const char* label, const bool selected, float x)
{
    ...
    if (selected)
        window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y), ImVec2(pos.x + x, pos.y + 30), ImColor(3, 3, 3));
    else
        window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y), ImVec2(pos.x + x, pos.y + 30), ImColor(19, 19, 19));

    window->DrawList->AddText({ bb.Min.x + size_arg.x / 2 - ImGui::CalcTextSize(label).x / 2, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(label).y / 2 }, ImColor(255, 255, 255), label);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
	...
	const ImVec4 icon_act = ImVec4(40 / 255.f, 40 / 255.f, 40 / 255.f, 1.f);

	RenderFrame(bb.Min, bb.Max, col, true, 2.f);

	window->DrawList->PushClipRect(bb.Min, bb.Max, false);
	ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(40 / 255.f, 40 / 255.f, 40 / 255.f, it_circle->second.alpha)), 60);
	window->DrawList->PopClipRect();

	ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(40 / 255.f, 40 / 255.f, 40 / 255.f, it_hover->second)), 2.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    col_bg = ImGui::GetColorU32(ImVec4(76 / 255.f, 143 / 255.f, 245 / 255.f, 120 / 255.f));
    col_bg2 = ImGui::GetColorU32(ImVec4(76 / 255.f, 143 / 255.f, 245 / 255.f, 255 / 255.f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(ImVec2(window->DC.CursorPos.x + 20, window->DC.CursorPos.y), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    //RenderTextOverlay(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f)) + ImVec2(-3, 10), value_buf[0], value_buf_end[0], false);
    window->DrawList->AddText(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 10) + ImVec2(-3, 0), ImColor(255, 255, 255), value_buf[0]);


    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5 + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x, frame_bb.Max.y - 5 - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(33, 33, 33), true, 15);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 5 + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f) + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, frame_bb.Max.y - 5 - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(26, 155, 255), true, g.Style.FrameRounding);
    //window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 7.f, ImColor(255, 255, 255), 64);


    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value. 
    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(frame_bb.Max.x + 10 - grab_bb.Max.x + grab_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y + 3.8), ImColor(255, 255, 255), label);
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
	...
	ImGui::NewLine(); ImGui::SameLine(42.f); ImGui::PushItemWidth(220.f);

	SameLine(window->Size.x - 76);

	ImGui::SetCursorPosX(ImGui::GetWindowSize().x - (ImGui::GetWindowSize().x - ImGui::CalcItemWidth()) + 17);
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		for (int n = 0; n < components; n++)
		{
			...
			if (!(flags & ImGuiColorEditFlags_NoOptions))
				OpenPopupContextItem("context", ImGuiColorEditFlags_NoOptions);
		}
	}
