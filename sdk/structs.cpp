...
bool player_t::isenemy()
{
	if (!this)
		return false;
	if (is_player() && is_alive())
	{
		if (g_ctx.local()->m_iTeamNum() != m_iTeamNum() && m_iTeamNum() != TEAM_GOTV)
			return true;

		if (m_cvar()->FindVar(("mp_teammates_are_enemies"))->GetInt() == 1)
			return true;
	}
...
float player_t::get_max_desync_delta1()
{
	if (!this)
		return 0.0f;
	auto animstate = get_animation_state();
