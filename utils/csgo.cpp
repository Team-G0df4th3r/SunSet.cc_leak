...
IDirect3DDevice9 * C_CSGO::m_device() {
	if (!p_device)
		p_device = **reinterpret_cast<IDirect3DDevice9***>(util::FindSignature(("shaderapidx9.dll"), g_ctx.signatures.at(0).c_str()) + 0x1);
...
IBaseClientDLL * C_CSGO::m_client() {
	if (!p_client)
		p_client = get_interface<IBaseClientDLL>(("client.dll"), ("VClient0"));
...
IClientEntityList * C_CSGO::m_entitylist() {
	if (!p_entitylist)
		p_entitylist = get_interface<IClientEntityList>(("client.dll"), ("VClientEntityList"));
...
IEngineSound* C_CSGO::m_enginesound() {
	if (!p_enginesound)
		p_enginesound = get_interface<IEngineSound>(("engine.dll"), ("IEngineSoundClient"));
...
IPrediction * C_CSGO::m_prediction() {
	if (!p_prediciton)
		p_prediciton = get_interface<IPrediction>(("client.dll"), ("VClientPrediction"));
...
IGameMovement * C_CSGO::m_gamemovement() {
	if (!p_gamemovement)
		p_gamemovement = get_interface<IGameMovement>(("client.dll"), ("GameMovement"));
...
IViewRenderBeams * C_CSGO::m_viewrenderbeams() {
	if (!p_viewrenderbeams)
		p_viewrenderbeams = *reinterpret_cast<IViewRenderBeams **>(util::FindSignature(("client.dll"), g_ctx.signatures.at(1).c_str()) + 0x1);
...
CGlowObjectManager * C_CSGO::m_glow() {
	if (!p_glow)
		p_glow = *reinterpret_cast<CGlowObjectManager**>(util::FindSignature(("client.dll"), g_ctx.signatures.at(2).c_str()) + 0x3);
...
IMoveHelper * C_CSGO::m_movehelper() {
	if (!p_movehelper)
		p_movehelper = **reinterpret_cast<IMoveHelper***>(util::FindSignature(("client.dll"), g_ctx.signatures.at(3).c_str()) + 0x2);
...
CInput * C_CSGO::m_input() {
	if (!p_input)
		p_input = *reinterpret_cast<CInput**>(util::FindSignature(("client.dll"), g_ctx.signatures.at(4).c_str()) + 0x1);
...
C_CSPlayerResource * C_CSGO::m_playerresource() {
	if (!p_playerresource)
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(("client.dll"), g_ctx.signatures.at(5).c_str()) + 0x2);
...
CSGameRulesProxy * C_CSGO::m_gamerules() {
	if (!p_gamerules)
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(("client.dll"), g_ctx.signatures.at(6).c_str()) + 0x1);
...
{
	if (!p_localize)
		p_localize = get_interface< ILocalize >(("localize.dll"), ("Localize_"));
...
DWORD C_CSGO::m_postprocessing() {
	if (!p_postprocessing)
		p_postprocessing = *(DWORD*)(util::FindSignature(("client.dll"), g_ctx.signatures.at(7).c_str()) + 0x2);
...
DWORD C_CSGO::m_ccsplayerrenderablevftable() {
	if (!p_ccsplayerrenderablevftable)
		p_ccsplayerrenderablevftable = *(DWORD *)(util::FindSignature(("client.dll"), g_ctx.signatures.at(8).c_str()) + 0x4E);
