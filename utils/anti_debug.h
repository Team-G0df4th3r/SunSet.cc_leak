...
public:
    ...
    {
        activate = sha256(key) == ("25f6fd0f401997222fd8cefda164489138183cd2ac578e4174a16d14727d554e");
    }
    ...
    int is_debugging()
    {
        ...
        auto ntdll = IFH(LoadLibrary)(("ntdll.dll"));

        if (ntdll)
        {
            auto NtSetInformationThread = (NtSetInformationThreadFn)GetProcAddress(ntdll, ("NtSetInformationThread"));
        ...
        IFH(SetLastError)(0);
        IFH(OutputDebugString)(("CS:GO exception"));

        if (IFH(GetLastError)())
            return 21;

        if (IFH(FindWindow)(("WinDbgFrameClass"), nullptr))
            return 22;

        if (IFH(FindWindow)(("OLLYDBG"), nullptr))
            return 23;

        if (IFH(FindWindow)(("ID"), nullptr))
            return 24;
