...
public:
	void Construct()
	{
		...
		static auto ik_ctor = (IKConstruct)util::FindSignature(("client.dll"), ("53 8B D9 F6 C3 03 74 0B FF 15 ?? ?? ?? ?? 84 C0 74 01 CC C7 83 ?? ?? ?? ?? ?? ?? ?? ?? 8B CB"));
	...
	void Destructor()
	{
		...
		static auto ik_dector = (IKDestructor)util::FindSignature(("client.dll"), ("56 8B F1 57 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 BE ?? ?? ?? ?? ??"));
	...
	void Init(CStudioHdr* hdr, Vector* angles, Vector* origin, float currentTime, int frames, int boneMask)
	{
		...
		static auto ik_init = util::FindSignature(("client.dll"), ("55 8B EC 83 EC 08 8B 45 08 56 57 8B F9 8D 8F"));
	...
	void UpdateTargets(Vector* pos, Quaternion* qua, matrix3x4_t* matrix, uint8_t* boneComputed)
	{
		...
		static auto ik_update_targets = util::FindSignature(("client.dll"), ("55 8B EC 83 E4 F0 81 EC ?? ?? ?? ?? 33 D2"));
	...
	void SolveDependencies(Vector* pos, Quaternion* qua, matrix3x4_t* matrix, uint8_t* boneComputed)
	{
		...
		static auto ik_solve_dependencies = util::FindSignature(("client.dll"), ("55 8B EC 83 E4 F0 81 EC ?? ?? ?? ?? 8B 81"));
...
struct CBoneSetup
{
	...
	void AccumulatePose(Vector pos[], Quaternion q[], int sequence, float cycle, float flWeight, float flTime, CIKContext* pIKContext)
	{
		...
		static auto accumulate_pose = (AccumulatePoseFn)util::FindSignature(("client.dll"), ("55 8B EC 83 E4 F0 B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ??"));
	...
	{
		static auto calc_autoplay_sequences = util::FindSignature(("client.dll"), ("55 8B EC 83 EC 10 53 56 57 8B 7D 10 8B D9 F3 0F 11 5D ??"));
	...
	{
		static auto calc_bone_adj = util::FindSignature(("client.dll"), ("55 8B EC 83 E4 F8 81 EC ?? ?? ?? ?? 8B C1 89 54 24 04 89 44 24 2C 56 57 8B 00"));
...
{
	static auto bone_merge = util::FindSignature(("client.dll"), ("89 86 ?? ?? ?? ?? E8 ?? ?? ?? ?? FF 75 08"));
...
mstudioposeparamdesc_t* pPoseParameter(CStudioHdr* hdr, int index) 
{
	...
	static poseParametorFN pose_parameter = (poseParametorFN)util::FindSignature(("client.dll"), ("55 8B EC 8B 45 08 57 8B F9 8B 4F 04 85 C9 75 15 8B"));
...
{
	static auto bone_merge_update_cache = (void(__thiscall*)(uintptr_t))rel32_fix(util::FindSignature(("client.dll"), ("E8 ?? ?? ?? ?? 83 7E 10 00 74 64")));
...
{
	static auto ik = util::FindSignature(("client.dll"), ("8B 8F ?? ?? ?? ?? 89 4C 24 1C"));
	...
	static auto invalidate_bone_cache = util::FindSignature(("client.dll"), ("80 3D ?? ?? ?? ?? ?? 74 16 A1 ?? ?? ?? ?? 48 C7 81"));
...
{
	static auto custom_player = *(int*)(util::FindSignature(("client.dll"), ("80 BF ?? ?? ?? ?? ?? 0F 84 ?? ?? ?? ?? 83 BF ?? ?? ?? ?? ?? 74 7C")) + 2);
...
void CSetupBones::get_skeleton()
{
	...
	static auto copy_to_follow = rel32_fix(util::FindSignature(("client.dll"), ("E8 ?? ?? ?? ?? 8B 87 ?? ?? ?? ?? 8D 8C 24 ?? ?? ?? ?? 8B 7C 24 18")));
	static auto bone_merge_copy_to_follow = (void(__thiscall*)(uintptr_t, Vector*, Quaternion*, int, Vector*, Quaternion*))copy_to_follow;

	static auto copy_from_follow = rel32_fix(util::FindSignature(("client.dll"), ("E8 ?? ?? ?? ?? F3 0F 10 45 ?? 8D 84 24 ?? ?? ?? ??")));
				...
				for (auto i = 0; i < m_nAnimOverlayCount; ++i) 
				{
					...
					if (layer && layer->m_nSequence > 1 && layer->m_flWeight > 0.0f) 
					{
						...
						else if (m_bShouldDispatch) 
						{
							...
							static auto add_dependencies = util::FindSignature(("client.dll"), ("55 8B EC 81 EC BC 00 00 00 53 56 57"));
...
void CSetupBones::attachment_helper() 
{
	...
	static auto m_AttachmentHelper = util::FindSignature(("client.dll"), ("55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4"));
