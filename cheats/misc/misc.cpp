...
#define VERSION ("SunSet.cc V2 Free Version")
...
void misc::watermark()
{
	...
	auto watermark = VERSION + g_ctx.username + (" | ") + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = VERSION + g_ctx.username + (" | ") + std::to_string(g_ctx.globals.ping) + (" ms | ") + tickrate + (" tick | ") + g_ctx.globals.time;
		}
	}

	auto box_width = render::get().text_width(fonts[NAME], watermark.c_str()) + 10;

	render::get().gradient(width - 10 - box_width, 10, box_width / 2, 1, Color(77, 7, 241), Color(7, 241, 159), GRADIENT_HORIZONTAL);
	render::get().gradient(width - 10 - box_width + (box_width / 2), 10, box_width / 2, 1, Color(7, 241, 159), Color(255, 94, 0), GRADIENT_HORIZONTAL);
	...
	render::get().gradient(width - 10 - box_width, 29, box_width / 2, 1, Color(77, 7, 241), Color(7, 241, 159), GRADIENT_HORIZONTAL);
	render::get().gradient(width - 10 - box_width + (box_width / 2), 29, box_width / 2, 1, Color(7, 241, 159), Color(255, 94, 0), GRADIENT_HORIZONTAL);
}
...
void misc::ChatSpamer()
{
	...
	{
		crypt_str("Stop being a boomer - get SunSet.CC."),
		crypt_str("Get good. Get SunSet.CC."),
		crypt_str("Stop missing, get SunSet.CC."),
		crypt_str("SunSet.CC - one step ahead of the game!. "),
	};
...
void misc::ViewModel()
{
	if (g_cfg.esp.viewmodel_fov)
	{
		...
		static auto viewFOVcvar = m_cvar()->FindVar(("viewmodel_fov"));
	...
	if (g_cfg.esp.viewmodel_x)
	{
		...
		static auto viewXcvar = m_cvar()->FindVar(("viewmodel_offset_x"));
	...
	if (g_cfg.esp.viewmodel_y)
	{
		...
		static auto viewYcvar = m_cvar()->FindVar(("viewmodel_offset_y"));
	...
	if (g_cfg.esp.viewmodel_z)
	{
		...
		static auto viewZcvar = m_cvar()->FindVar(("viewmodel_offset_z"));
...
void misc::FullBright()
{
	...
	static auto mat_fullbright = m_cvar()->FindVar(("mat_fullbright"));
...
void misc::NightmodeFix()
{
	...
	if (m_engine()->IsInGame() && !in_game)
	{
		...
		static auto skybox = m_cvar()->FindVar(("sv_skyname"));
...
void misc::rank_reveal()
{
	...
	static auto Fn = (RankReveal_t)(util::FindSignature(("client.dll"), ("55 8B EC 51 A1 ? ? ? ? 85 C0 75 37")));
