...
public:
	bool OnMapLoad(const char* pszMapName, IDirect3DDevice9* pDevice)
	{
		...
		auto bombA_x = parseString(("\"bombA_x\""), szInfo);

		if (!bombA_x.empty())
		{
			bomb_siteA.x = std::stod(parseString(("\"bombA_x\""), szInfo));
			bomb_siteA.y = std::stod(parseString(("\"bombA_y\""), szInfo));
			bomb_siteA.z = 0.0f;

			draw_plantA = true;
		}

		auto bombB_x = parseString(("\"bombB_x\""), szInfo);

		if (!bombB_x.empty())
		{
			bomb_siteB.x = std::stod(parseString(("\"bombB_x\""), szInfo));
			bomb_siteB.y = std::stod(parseString(("\"bombB_y\""), szInfo));
			bomb_siteB.z = 0.0f;
