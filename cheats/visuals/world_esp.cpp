...
{
	static auto load_skybox = reinterpret_cast<void(__fastcall*)(const char*)>(util::FindSignature(("engine.dll"), ("55 8B EC 81 EC ? ? ? ? 56 57 8B F9 C7 45")));
...
{
	static auto fog_override = m_cvar()->FindVar(("fog_override")); //-V807
	...
	static auto fog_start = m_cvar()->FindVar(("fog_start"));

	if (fog_start->GetInt())
		fog_start->SetValue(0);

	static auto fog_end = m_cvar()->FindVar(("fog_end"));

	if (fog_end->GetInt() != g_cfg.esp.fog_distance)
		fog_end->SetValue(g_cfg.esp.fog_distance);

	static auto fog_maxdensity = m_cvar()->FindVar(("fog_maxdensity"));
	...
	static auto fog_color = m_cvar()->FindVar(("fog_color"));
...
void worldesp::bomb_timer(entity_t* entity)
{
	...
	static auto mp_c4timer = m_cvar()->FindVar(("mp_c4timer"));
