...
bool can_penetrate(weapon_t* weapon)
{
	...
	static auto damageReductionBullets = m_cvar()->FindVar(("ff_damage_reduction_bullets"));
	static auto damageBulletPenetration = m_cvar()->FindVar(("ff_damage_bullet_penetration"));
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DFilledCircle(position, alpha * 20.f, g_ctx.globals.fired_shot ? Color(40, 220, 5, (int)(alpha * 135.0f)) : Color(172, 231, 195, (int)(alpha * 125.0f)));
		render::get().Draw3DCircle(position, alpha * 20.f, g_ctx.globals.fired_shot ? Color(60, 240, 25, (int)(alpha * 135.0f)) : Color(114, 246, 215, (int)(alpha * 125.0f)));
