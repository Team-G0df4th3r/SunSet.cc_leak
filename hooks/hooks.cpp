...
namespace hooks
{
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		if (bg == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(g_pd3dDevice, &background, sizeof(background),
				670, 412,
		...
		//c_menu::get().skeet_icon = io.Fonts->AddFontFromMemoryTTF((void*)skeet, sizeof(skeet), 40.f, &m_config, ranges);
		//c_menu::get().skeet_menu = io.Fonts->AddFontFromMemoryTTF((void*)railway, sizeof(railway), 13.f, &m_config, ranges);
		
		//c_menu::get().keybind_font = io.Fonts->AddFontFromMemoryTTF((void*)skeet_font, sizeof(skeet_font), 12.f, &m_config, ranges);
		c_menu::get().info = io.Fonts->AddFontFromFileTTF("C:/windows/fonts/arial.ttf", 14);
		c_menu::get().info_little = io.Fonts->AddFontFromFileTTF("C:/windows/fonts/arial.ttf", 11);
		c_menu::get().ee = io.Fonts->AddFontFromFileTTF("C:/windows/fonts/arial.ttf", 13.0f, &m_config, ranges);
		c_menu::get().two = io.Fonts->AddFontFromFileTTF("C:/windows/fonts/arialbd.ttf", 20);
		c_menu::get().three = io.Fonts->AddFontFromFileTTF("C:/windows/fonts/impact.ttf", 50);


		if (c_menu::get().logggo == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(
				...
				NULL, (D3DXIMAGE_INFO*)NULL,
				(PALETTEENTRY*)NULL, &c_menu::get().logggo);
		}

		if (c_menu::get().RGBline == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(
				pDevice,
				&RGBLINE, sizeof(RGBLINE),
				580, 140,
				D3DX_DEFAULT, 0,
				D3DFMT_UNKNOWN, D3DPOOL_DEFAULT,
				D3DX_DEFAULT, D3DX_DEFAULT,
				NULL, (D3DXIMAGE_INFO*)NULL,
				(PALETTEENTRY*)NULL, &c_menu::get().RGBline);
		}
		c_menu::get().weapon_font = io.Fonts->AddFontFromMemoryTTF((void*)WeaponzFont, sizeof(WeaponzFont), 17.f, &m_config, ranges);
