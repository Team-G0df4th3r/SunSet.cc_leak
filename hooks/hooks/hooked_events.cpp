...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	{
		auto userid = m_engine()->GetPlayerForUserID(event->GetInt(("userid")));
		...
		if (e->valid(false, false))
		{
			...
			{
				static auto model_index = m_modelinfo()->GetModelIndex(("sprites/physbeam.vmt"));

				if (g_ctx.globals.should_update_beam_index)
					model_index = m_modelinfo()->GetModelIndex(("sprites/physbeam.vmt"));
	...
	else if (!strcmp(event_name, ("weapon_fire")))
	{
		auto user_id = event->GetInt(("userid"));
	...
	else if (!strcmp(event_name, ("bullet_impact")))
	{
		auto user_id = event->GetInt(("userid"));
	...
	else if (!strcmp(event_name, ("player_death")))
	{
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (g_cfg.esp.killsound)
			{
				...
				{
				case 0:
					m_surface()->PlaySound_(headshot ? ("kill2.wav") : ("kill1.wav"));
					break;
				case 1:
					m_surface()->PlaySound_(("kill3.wav"));
					break;
				case 2:
					m_surface()->PlaySound_(("kill4.wav"));
					break;					
				case 3:						
					m_surface()->PlaySound_(("kill5.wav"));
					break;					
				case 4:						
					m_surface()->PlaySound_(("kill6.wav"));
					break;				
				case 5:					
					m_surface()->PlaySound_(("kill7.wav"));
					break;				
				case 6:					
					m_surface()->PlaySound_(("kill8.wav"));
					break;				
				case 7:					
					m_surface()->PlaySound_(("kill9.wav"));
					break;					
				case 8:						
					m_surface()->PlaySound_(("kill10.wav"));
					break;				
				default:				
					m_surface()->PlaySound_(("kill11.wav"));
					break;
				}
			}
			...
			if (weapon && weapon_name.find(("knife")) != std::string::npos)
				SkinChanger::overrideHudIcon(event);
		}
	}
	else if (!strcmp(event_name, ("player_hurt")))
	{
		auto attacker = event->GetInt(("attacker"));
		auto user = event->GetInt(("userid"));
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (g_cfg.esp.hitsound && g_cfg.player.enable)
			{
				...
				{
				case 1:
					m_surface()->PlaySound_(("metallic.wav"));
					break;
				case 2:
					m_surface()->PlaySound_(("cod.wav"));
					break;
				case 3:
					m_surface()->PlaySound_(("bubble.wav"));
					break;
				case 4:
					m_surface()->PlaySound_(("stapler.wav"));
					break;
				case 5:
					m_surface()->PlaySound_(("bell.wav"));
					break;
				case 6:
					m_surface()->PlaySound_(("flick.wav"));
					break;
				}
			}
			...
			if (current_shot)
			{
				current_shot->shot_info.result = ("Hit");
	...
	else if (!strcmp(event_name, ("bomb_defused")))
...
void C_HookedEvents::RegisterSelf()
{
	...
	eventmanager->AddListener(this, ("player_footstep"), false);
	eventmanager->AddListener(this, ("player_hurt"), false);
	eventmanager->AddListener(this, ("player_death"), false);
	eventmanager->AddListener(this, ("weapon_fire"), false);
	eventmanager->AddListener(this, ("item_purchase"), false);
	eventmanager->AddListener(this, ("bullet_impact"), false);
	eventmanager->AddListener(this, ("round_start"), false);
	eventmanager->AddListener(this, ("round_freeze_end"), false);
	eventmanager->AddListener(this, ("bomb_defused"), false);
	eventmanager->AddListener(this, ("bomb_begindefuse"), false);
	eventmanager->AddListener(this, ("bomb_beginplant"), false);
