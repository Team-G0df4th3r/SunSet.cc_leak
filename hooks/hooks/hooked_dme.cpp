...
IMaterial* CreateMaterial(bool lit, const std::string& material_data)
{
	...
	auto matname = ("SunSet.CC_") + std::to_string(created);
	++created;

	auto keyValues = new KeyValues(matname.c_str());
	static auto key_values_address = util::FindSignature(("client.dll"), ("55 8B EC 51 33 C0 C7 45"));

	using KeyValuesFn = void(__thiscall*)(void*, const char*);
	reinterpret_cast <KeyValuesFn> (key_values_address)(keyValues, type.c_str());

	static auto load_from_buffer_address = util::FindSignature(("client.dll"), ("55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89"));
