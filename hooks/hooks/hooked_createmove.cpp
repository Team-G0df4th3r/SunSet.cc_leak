...
bool __stdcall hooked_createmove(float sample_time, CUserCmd* m_pcmd, bool& bSendPacket)
{
	...
	if (g_ctx.globals.tickbase_shift)
		g_ctx.globals.fixed_tickbase = g_ctx.local()->m_nTickBase() - g_cfg.ragebot.double_tap_shift_value;
	else
	...
	if (m_pcmd->m_buttons & IN_ATTACK2 && g_cfg.ragebot.enable && g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER)
		m_pcmd->m_buttons &= ~IN_ATTACK2;
	if (g_ctx.globals.tickbase_shift) {
		g_ctx.send_packet = g_ctx.globals.tickbase_shift == 1;
...
//fixed by team-g0df4th3r cuz sunset devs are actually too retarded to do anything
