...
void weather()
{
	...
	static auto m_nPrecipType = netvars::get().get_offset(("CPrecipitation"), ("m_nPrecipType"));
	static auto m_vecMins = netvars::get().get_offset(("CBaseEntity"), ("m_vecMins"));
	static auto m_vecMaxs = netvars::get().get_offset(("CBaseEntity"), ("m_vecMaxs"));
...
void remove_smoke()
{
	...
	{
		static auto smoke_count = *reinterpret_cast<uint32_t**>(util::FindSignature(("client.dll"), ("A3 ? ? ? ? 57 8B CB")) + 0x1);
		*(int*)smoke_count = 0;
	}
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	{
		static auto r_drawspecificstaticprop = m_cvar()->FindVar(("r_drawspecificstaticprop")); //-V807
		...
		static auto cl_foot_contact_shadows = m_cvar()->FindVar(("cl_foot_contact_shadows")); //-V807

		if (cl_foot_contact_shadows->GetBool())
			cl_foot_contact_shadows->SetValue(FALSE);

		static auto zoom_sensitivity_ratio_mouse = m_cvar()->FindVar(("zoom_sensitivity_ratio_mouse"));
		...
		static auto r_modelAmbientMin = m_cvar()->FindVar(("r_modelAmbientMin"));
		...
		if (current_shot != g_ctx.shots.end())
		{
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					{
						current_shot->shot_info.result = ("Resolver");
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().add(("!!Miss |Missed shot due to Resolver (failed to resolve)" ));
#endif
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						current_shot->shot_info.result = ("Spread");

						if (current_shot->occlusion)
							eventlogs::get().add(("!!Miss |Missed shot due to occlusion"));
						else if (current_shot->shot_info.hitchance == 100)
							eventlogs::get().add(("!!Miss |Missed shot due to error in prediction system"));
						else
							eventlogs::get().add(("!!Miss |Missed shot due to spread (inaccuracy)"));
					}
				}
			}
	...
	{
		if (!death_notice)
			death_notice = util::FindHudElement <DWORD>(("CCSGO_HudDeathNotice"));
		...
		if (death_notice)
		{
			...
			if (g_ctx.globals.should_clear_death_notices)
			{
				...
				static auto clear_notices = (Fn)util::FindSignature(("client.dll"), ("55 8B EC 83 EC 0C 53 56 8B 71 58"));
